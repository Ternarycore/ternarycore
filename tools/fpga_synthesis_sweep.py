...
def synthesize(*, sources: list[Path], top: str, params: dict[str, int],
    ...
    lines = [f"read_verilog -sv {path.resolve()}" for path in sources]
        ...
        if source:
            row["synthesis"] = synthesize(
                ...
                params={"DATA_WIDTH": candidate.data_width,
                        "ACC_WIDTH": candidate.acc_width}, yosys=yosys)
        rows.append(row)
    return rows
