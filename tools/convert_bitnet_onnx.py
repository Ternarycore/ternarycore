...
def generate_tests(rows: int, cols: int, rng_seed: int = 42):
    ...
    for row in range(rows):
        ...
        # Compute expected output: accumulate dot over depth, then scale per col
        dot_acc = [0] * cols
        for d in range(depth):
            for c in range(cols):
                dot_acc[c] += activations[d] * all_weights[d][c]
        expected = [dot * alphas[c] // 32768 for c, dot in enumerate(dot_acc)]
