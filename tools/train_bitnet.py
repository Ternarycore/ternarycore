#!/usr/bin/env python3
"""
BitNet b1.58 fine-tuning recipe for SmolLM2-256M.
...
def flatten_model(model):
    """Recursively flatten named modules for optimizer."""
    params = []
...
def train_epoch(model, embed, loader, optimizer, scheduler, device):
    model.train()
    total_loss = 0
    n_batches = 0
    ...
    for batch in loader:
        ...
        logits = model(batch).last_hidden_state @ embed.T
        loss = F.cross_entropy(logits[:, :-1, :].reshape(-1, logits.shape[-1]),
                               batch[:, 1:].reshape(-1))
        
        loss.backward()
        torch.nn.utils.clip_grad_norm_(flatten_model(model), 1.0)
        optimizer.step()
        scheduler.step()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        train_loss = train_epoch(model.text_model, model.text_model.embed_tokens.weight,
                                 loader, optimizer, scheduler, device)
        
        # Evaluate
        model.eval()
        with torch.no_grad():
            ppl = compute_ppl(model.text_model, model.text_model.embed_tokens.weight,
                           val_data.to(device))
        ...
        torch.save({
            'epoch': epoch,
            'model_state': model.text_model.state_dict(),
        ...
        if ppl < best_ppl:
            best_ppl = ppl
            torch.save(model.text_model.state_dict(), args.output / "best.pt")
