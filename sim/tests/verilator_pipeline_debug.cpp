...
int main() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        printf("IN %d: act=%d q_valid=%d gemm_valid=%d scale_valid=%d\n",
               i, dut->activation, dut->quant->valid_out, dut->gemm->valid_out, dut->scale->valid_out);
    }

    dut->valid_in = 0;
    for (int i = 0; i < 15; i++) {
        dut->clk = 0; dut->eval(); dut->clk = 1; dut->eval();
        printf("OUT %d: valid_out=%d res=[%d,%d,%d,%d]\n",
               i, dut->valid_out,
