...
// ternary_gemm → ternary_scale), cover + bmc.
//
// Reference model: a structurally-different shadow of the whole pipeline,
// built from algebraic operations and sliding-window sums rather than the
// RTL's counter state machine:
//   * quantization: q = clip((x * inv + 2^14) >>> 15, -127, +127), latched
//     through two gated stages (x_d1 when valid_in, x_d2 when v_d1), so the
//     gemm consumes q(valid) and feed = valid_in delayed 2 cycles.
//   * weight alignment: weight_enc_d2 is an unconditional 2-stage shift in
//     the RTL, so the reference delays weight_enc by 2 cycles the same way.
//     The assertion below therefore catches alignment skew between the
//     quantized activation stream and the weight stream (the PR#11 fix).
//   * gemm: per-column running sum over a VECTOR_LEN-deep window of ±q
//     products; on the 4th feed of a window the sum is latched and a one-
//     cycle done pulse is emitted, mirroring the dot's down-counter.
//   * scale: prod <= acc * alpha, then scale_ch (truncate 15 frac bits,
//     round up if any set, arithmetic >>15), latched one cycle later.
//
// Asserts (gated on run_cnt >= 4 so reset has fully flushed):
//   * dut.valid_out == ref.svalid   — exact output-pulse timing; catches
//     sticky/extra/missing valid_out and per-column skew.
//   * dut.valid_out == 0 || dut.result[col] == ref.result[col] per column —
//     the result must be correct whenever a result is presented.

`timescale 1ns / 1ps

module ternary_pipeline_formal #(
    ...
    always @(posedge clk) begin
        if (reset_cnt < 2) begin
            ...
            assume(rst_n);
            // inv is a precomputed per-tensor reciprocal held constant during
            // a pass (absmax=100 -> inv=41615, as in activation_quant_formal).
            assume(inv == 22'd41615);
            // alpha is a per-tensor scale, also held constant during a pass.
            // Q15 unsigned max is 65535 (alpha=2.0); 65536 would wrap to 0.
            assume(alpha[0*(PRECISION+1) +: PRECISION+1] == 16'd32768);
            assume(alpha[1*(PRECISION+1) +: PRECISION+1] == 16'd65535);
            // Bounded activation range. This formal's job is the datapath
            // wiring/timing (quantizer latency vs weight_enc_d2 alignment,
            // valid_out pulsing, back-to-back vectors); the quantize and
            // scale multiplier arithmetic is already proven for ALL inputs by
            // activation_quant_formal and ternary_scale_formal. Bounding the
            // activation keeps the unrolled multiplier chains solver-friendly
            // (the pipeline prove runs in ~seconds instead of hours) while
            // leaving valid_in, weight_enc and the alignment logic fully free.
            assume(activation >= -8'sd20 && activation <= 8'sd20);
        end
    end
    ...
    wire signed [DATA_WIDTH+INV_WIDTH-1:0] ref_prod    = $signed(x_d2) * $signed({1'b0, inv});
    ...
    // If the pipeline drifts into an invalid state, valid_out could glitch
    // or stick. When valid_in is low, valid_out must not hold for >1 cycle.
    always @(posedge clk) begin
