...
module ternary_scale #(
    ...
    reg [ACC_WIDTH+PRECISION:0] prod_0, prod_1, prod_2, prod_3;

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            prod_0 <= 0; prod_1 <= 0; prod_2 <= 0; prod_3 <= 0;
        end else if (valid_in) begin
            prod_0 <= $signed(acc_in[0*ACC_WIDTH +: ACC_WIDTH]) * $signed({1'b0, alpha[0*(PRECISION+1) +: PRECISION+1]});
            prod_1 <= $signed(acc_in[1*ACC_WIDTH +: ACC_WIDTH]) * $signed({1'b0, alpha[1*(PRECISION+1) +: PRECISION+1]});
            prod_2 <= $signed(acc_in[2*ACC_WIDTH +: ACC_WIDTH]) * $signed({1'b0, alpha[2*(PRECISION+1) +: PRECISION+1]});
            prod_3 <= $signed(acc_in[3*ACC_WIDTH +: ACC_WIDTH]) * $signed({1'b0, alpha[3*(PRECISION+1) +: PRECISION+1]});
        end
    end
