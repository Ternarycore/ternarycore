...
- Pre-tokenizes the corpus once to ~/tc-data/wt103.pt (reused on restart).
...
if os.path.exists(ids_path):
    ...
    print("[warmup] tokenizing wikitext-103 (one-time)...", flush=True)
    text = "\n\n".join(load_dataset("wikitext", "wikitext-103-raw-v1",
                                     split="train")["text"])
    ids = tok(text, return_tensors="pt").input_ids[0].to(torch.int32)
...
sched = torch.optim.lr_scheduler.LambdaLR(
    ...
    (1 + torch.cos(torch.tensor(min(s, total_steps) / total_steps * 3.14159)).item()))
...
for step in range(start_step, total_steps):
    ...
    for a in range(args.accum):
        ...
        (model(x, labels=x).loss / args.accum).backward()
    torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
    opt.step(); sched.step()
    if step % 25 == 0:
        done = (step - start_step + 1) * tok_per_step
        rate = done / max(time.time() - t0, 1)
        print(f"MARK step {step}/{total_steps} "
              f"loss {model(x, labels=x).loss.item():.4f} "
